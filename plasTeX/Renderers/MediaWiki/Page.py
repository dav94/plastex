...
class Page(object):
	...
	def collapseText(self,level,pages_dict,media_url,last_url,url_dic):
		if(self.level<level):
			last_url = self.url
			for subpage in self.index:
				pages_dict[subpage].collapseText(level,pages_dict,media_url,last_url,url_dic)
